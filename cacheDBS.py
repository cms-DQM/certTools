#!/usr/bin/env python
import os, time, shutil, string, subprocess,sys, glob, commands, json

outfile = "DBS2012D_cache_TEST.txt"
formatjson = True
use_das    = False
...
if __name__ == "__main__":
    ...
    sdbs = '' 
    if use_das:
       sdbs = './das_client.py --query="run,lumi dataset=/MinimumBias/Run2012D-v1/RAW run=RUNNUMBER system=dbs3" --format=json --das-headers --limit=0'
    else:
    ...
    for run in mylist:
        if run.isdigit():
            ...
            if not retval == 0:
                ...
                if "das_client" not in thisProcess:  
		    print " Successed! "
		    ...
		    outptr.close()
	  	    for line in outData:
			    ...
			    if len(line.split()) > 1:
                                if formatjson:
			            thislumi[run].append(int(line.split()[1]))
				else:
		        ...
		        if len(lumirange)==1:
	                    lumirange.append(lastlumi)
		            lumiranges.append(lumirange)
                        thisrunlumi += ' '.join([str(item) for item in lumiranges]) 

   

		else:
                    (status, out) = commands.getstatusoutput(thisProcess)
                    if status:
                    ...
                    for l in lumi:
                        if formatjson: 
                            ...
                            for ls in range(l[0], l[len(l)-1]+1):
                                thisrunlumi += str(ls) + " "
